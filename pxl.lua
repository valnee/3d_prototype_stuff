...
function pxl.restore(PXL, bg)
    PXL.canvas = create_buffer(PXL.width, PXL.height, bg or colors.black)
end

function pxl.restore_chars(PXL, char, fg, bg)
    PXL.char_canvas = create_buffer(PXL.s_width, PXL.s_height, {char or "", fg or colors.white, bg or colors.black})
end
...
function pxl.new(term, background)
    ...
    self.s_width, self.s_height = term.getSize()
    self.width, self.height = self.s_width*2, self.s_height*3
...
function pxl:is_in_bounds(x,y)
    return (x >= 2 and x <= self.width-1) and (y >= 2 and y <= self.height-1)
end
